/*!
 * wavesurfer.js timeline plugin 3.1.0 (2019-09-29)
...
function () {
  ...
  _createClass(TimelinePlugin, [{
    ...
    value: function renderCanvases() {
      ...
      var start = this.params.start; // if parameter is function, call the function with
      ...
      renderPositions(function (i, curSeconds, curPixel) {
        if (i % primaryLabelInterval === 0) {
          ...
          _this3.fillText(formatTime(curSeconds, pixelsPerSecond, start), curPixel + _this3.params.labelPadding * _this3.pixelRatio, height1);
        }
    ...
    value: function defaultFormatTimeCallback(seconds, pxPerSec, start) {
      var seconds = seconds + start
      // if (seconds / 60 > 1) {
      ...
      var tt = new Date(parseInt(seconds) * 1000).toLocaleString('chinese', { hour12: false });
        var arr = tt.split(' ');
        ...
        var tmp = (arr1.join('-') + ' ' + arr[1]).substr(11)
        return tmp
    }
    /**
