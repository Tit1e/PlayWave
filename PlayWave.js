...
(function(global, factory) {
  ...
  // 如果是点击波形跳转引发的临时性暂停，此值会临时设为 true, 200ms 后重新设置为 false，
  let seekPause = false;
  // m3u8 预期开始时间
  let startTime = 0;
  // 播放器配置
  ...
  class PlayWave {
    ...
    // 波形实例
    wavesurfer = null;
    constructor(_playerOptions, _waveSurferOptions, _timeLineOptions, _options) {
      // 储存配置
      ...
      return new Promise((resolve, reject) => {
        this._http(url)
          .then((data) => {
            ...
            const arr = r.split("/");
            const m3u8Start = startTime
            const seek = m3u8Start - Number(arr.pop().substring(0, 10));
            resolve(seek);
          })
          .catch(() => {
            reject();
    ...
    on(key, callback) {
      if (!this.pKEY_LIST.includes(key) && !this.wKEY_LIST.includes(key)) {
